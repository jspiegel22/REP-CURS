...
const TestSubmissionPage: NextPage = () => {
  ...
  // Function to fetch submissions (only works if admin API is available)
  const fetchSubmissions = async () => {
    setLoading(true);
    try {
      const response = await fetch('/api/admin/guide-submissions');
      if (response.ok) {
  ...
  const testDirectSubmission = async () => {
    ...
    try {
      ...
      const testData = {
        ...
        interestAreas: ['villas', 'activities'],
  ...
  return (
    <>
      <Head>
        <title>Guide Submission Testing | Cabo Travel Platform</title>
        <meta name="description" content="Test the guide submission functionality" />
      </Head>
                ...
                {submissions.length > 0 ? (
                  ...
                  <div className="text-center p-6 text-gray-500">
                    {loading ? (
                      ...
                      <p>No submissions found or admin API access is restricted</p>
                    )}
                  </div>
                )}
              </TabsContent>
