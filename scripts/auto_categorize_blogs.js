...
// Sample blog content to categorize
const blogPosts = [
  {
...
// Function to categorize blog posts using OpenAI
async function categorizeBlogPost(post) {
  try {
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "You are a blog categorization assistant for a Cabo San Lucas travel website. Your task is to analyze blog content and categorize it into one of these categories: 'Local Guides', 'Luxury Travel', 'Weddings', 'Dining', 'Family Travel', 'Activities', 'Events'. Also suggest 3-5 relevant tags for each post. Respond with JSON in this format: { 'category': string, 'tags': string[] }"
        },
        {
          role: "user",
          content: `Please categorize this blog post:\nTitle: ${post.title}\nExcerpt: ${post.excerpt}\nContent: ${post.content}`
        }
      ],
    ...
    // Add the categorization results to the post
    return {
      ...post,
    ...
    console.error(`Error categorizing post "${post.title}":`, error);
    // Return post with default category and tags if there's an error
    return {
      ...post,
      slug: createSlug(post.title),
      category: "Travel",
...
async function main() {
  console.log("Starting to categorize blog posts...");
  
  // Create array to store categorized posts
  const categorizedPosts = [];
  
  // Process each blog post
  for (const post of blogPosts) {
    console.log(`\nProcessing post: "${post.title}"`);
    const categorizedPost = await categorizeBlogPost(post);
    categorizedPosts.push(categorizedPost);
    
    console.log(`Slug: ${categorizedPost.slug}`);
    console.log(`Category: ${categorizedPost.category}`);
    console.log(`Tags: ${categorizedPost.tags.join(', ')}`);
    console.log("--------------------------------------------");
  }
  
  // Save the categorized posts to a file
  await fs.writeFile(
    'categorized_blog_posts.json', 
    JSON.stringify(categorizedPosts, null, 2)
  );
  
  console.log("\nSummary of blog categorization:");
  console.log("--------------------------------------------");
  categorizedPosts.forEach(post => {
    console.log(`${post.slug} - Category: ${post.category}`);
  });
  
  console.log("\nCategorization complete! Results saved to categorized_blog_posts.json");
}
