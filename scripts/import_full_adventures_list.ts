...
function determineCategory(title: string): 'water' | 'land' | 'luxury' | 'family' | 'yacht' {
...
function generateDescription(title: string, price: string, duration: string, minAge: string | null): string {
...
function generateKeyFeatures(title: string, category: string): string[] {
  const baseFeatures = [
    'Professional guide',
...
function generateThingsToBring(category: string): string[] {
  const commonItems = [
    'Sunscreen',
    'Sunglasses',
    'Camera',
...
function isTopRecommended(title: string, price: string | null): boolean {
...
async function extractDataFromCSV(filePath: string) {
  try {
    ...
    const adventures = dataRows.map((row: any) => {
