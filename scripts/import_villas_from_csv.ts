/**
 * This script imports villas from a CSV file into the database
 */
...
async function extractVillasFromCSV(filePath: string) {
  try {
    ...
    const villas = dataRows.map((row: any, index: number) => {
      // URL and image
      const url = row[0] || '';
      ...
      // Generate unique trackHsId based on the name
      const trackHsId = `hs-${generateSlug(name)}-${index}`;
    ...
    console.log(`Extracted ${villas.length} villas from CSV`);
    return villas;
  } catch (error) {
...
// Function to handle the JSONB conversion
function to_jsonb(arr: any[]): any {
  return arr;
}
...
async function importVillas() {
  try {
    console.log('Starting villa import process...');
    
    // Get villas from CSV
    const extractedVillas = await extractVillasFromCSV('./attached_assets/cabovillas-2025-03-08.csv');
    
    console.log(`Processing ${extractedVillas.length} villas...`);
    ...
    // Process each villa
    for (const villa of extractedVillas) {
      try {
...
(async function () {
  try {
    const result = await importVillas();
